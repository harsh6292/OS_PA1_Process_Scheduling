...
int	queueA_head, queueA_tail;
int	queueB_head, queueB_tail;
int	queueC_head, queueC_tail;
int	queueD_head, queueD_tail;
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	queueA_tail = 1 + (queueA_head=newqueue());
	queueB_tail = 1 + (queueB_head=newqueue());
	queueC_tail = 1 + (queueC_head=newqueue());
	queueD_tail = 1 + (queueD_head=newqueue());
